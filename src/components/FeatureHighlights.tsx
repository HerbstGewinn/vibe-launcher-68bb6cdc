  ...
  return <div className={cn('py-0', className)}>
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {features.map((feature, index) => <motion.div key={index} className={cn('frost-container p-6 cursor-pointer transition-all duration-500 group', activeFeature === index ? 'border-neon shadow-neon' : 'hover:border-neon/40 hover:shadow-neon-sm')} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
          ...
          once: true
              ...
              <div className={cn('overflow-hidden transition-all duration-500', activeFeature === index ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0')}>
                <div className="pt-4 border-t border-slate-700 mt-2">
                  <h4 className="font-medium mb-3">Key Benefits:</h4>
                  <ul className="space-y-2">
                    {feature.benefits.map((benefit, bIndex) => <li key={bIndex} className="flex items-start gap-2">
